...
def create_concept_cooccurrence_graph():
    ...
    # Load concept edges — rebuild from raw pkl if CSV is empty
    df = None
    ...
    if df is None or len(df) == 0:
        ...
        for _, work in raw_df.iterrows():
            work_id = work.get("id")
            cs_list = work.get("concepts_scores") or work.get("concepts") or []
            ...
            for cs in cs_list:
                if isinstance(cs, dict):
                    ...
                    concept_name = str(cs)
                if concept_name:
                    rows.append({"paper_id": work_id, "concept_id": concept_name.lower(),
    ...
    if norm_needed:
        ...
        G_norm = nx.Graph()
        for (c1, c2), raw_w in raw_edge_counts.items():
...
def create_field_sharing_graph():
    ...
    if norm_needed:
        ...
        G_norm = nx.Graph()
        for (f1, f2), raw_w in raw_edge_counts.items():
...
if __name__ == "__main__":
    ...
    print(f"\nAll Dimensions graphs processed using '{EXTRACTION_MODE}' mode. Saved to nx_graphs/.")
